...
def card_dashboard():
    ...
    response_last_upload = get_latest_uploaded_data()
    last_upload = response_last_upload.json
    last_upload_data = last_upload.get('latestUploadedData')

    datetime_obj = datetime.strptime(last_upload_data, '%Y-%m-%dT%H:%M:%S')
    last_upload_data_formated = datetime_obj.strftime('%d/%m/%Y %H:%M:%S')
    return render_template('card-dashboard.html', data_total=data_total, last_upload_data=last_upload_data_formated)
